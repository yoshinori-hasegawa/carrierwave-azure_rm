    ...
    class AzureRM < Abstract
      ...
      def connection
        @connection ||= begin
          ...
          #  ::Azure::Storage::Common::Configurable.send("#{key}=", uploader.send("azure_#{key}"))
          #end
          ::Azure::Storage::Blob::BlobService.new(storage_account_name: uploader.azure_storage_account_name, storage_access_key: uploader.azure_storage_access_key)
        end
      end

      def signer
        @signer ||= begin
          ::Azure::Storage::Common::Core::Auth::SharedAccessSignature.new(uploader.azure_storage_account_name, uploader.azure_storage_access_key)
        end
      end
